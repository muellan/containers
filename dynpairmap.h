...
class dynpairmap
{
    ...
    class iter_t_
    {
        using traits_t_ = std::iterator_traits<Iter>;
    public:
        //---------------------------------------------------------------
        ...
        //-----------------------------------------------------
        explicit constexpr
        iter_t_(Iter start, difference_type num = 0):
            p_(start), mark_(start + num - 1), count_(num), stride_(2)
        {}
        //-----------------------------------------------------
        explicit constexpr
        iter_t_(Iter start, difference_type num, difference_type offset):
    ...
    struct section_t_
    {
        ...
        struct iterator
        {
            using size_type = typename storage_t_::size_type;
            using traits_t_ = std::iterator_traits<Iter>;
        public:
            //-------------------------------------------
            using iterator_category = std::forward_iterator_tag;
...
public:
    ...
    public:
        ...
        restore(dynpairmap& target, size_type index) const {
            ...
            for(size_type i = index+1; i < target.vals_.cols(); ++i) {
    ...
    push_up(size_type firstIndex, size_type by = 1) {
        ...
        if(max_index() < 1) {
            ...
            vals_.insert_rows(firstIndex, by);
            vals_.insert_cols(firstIndex, by);
        }
    }
    //-----------------------------------------------------
    void
    push_up(
        size_type firstIndex, size_type by, const value_type& value)
    {
        if(by < 1) return;

        push_up(firstIndex,by);

        for(size_type j = firstIndex; j < firstIndex+by; ++j) {
